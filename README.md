...
You'll need to set up an App Registration in Azure AD (under the directory you are using). Choose the 'Native' type and use https://portal.azure.com as the redirect URI. It only needs the default "Sign in and read user profile" permission for standard authentication.

Once setup, you can grant permissions in required permissions in the app registration in the portal or run `/etc/openvpn/openvpn-azure-ad-auth.py --consent` inside the container (once-off). Before you run the command, export your Azure `username` and `password` to the environment, then follow the link in the stdout, it should read something like `To sign in, use a web browser to open the page https://aka.ms/devicelogin and enter the code HJW9PMABC to authenticate.`.
